...
export default function BuybackEmailNotificationsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await adminEmailApi.getBuybackAutoSend()
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      const res = await adminEmailApi.updateBuybackAutoSend(settings)
      setSettings(res.data.settings)
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      <h1 className="text-2xl font-bold mb-2">買取メール — 自動送信設定</h1>
