          ...
          <>
            ...
            {paymentDeadline && (
              <p className="text-amber-600 text-sm font-medium">
                {lang === 'ja' ? 'お支払い期限' : 'Payment deadline'}:{' '}
                {new Date(paymentDeadline).toLocaleString(lang === 'ja' ? 'ja-JP' : 'en-US')}
              </p>
            )}
            <p className="text-orange-600/90 text-xs">
              {lang === 'ja'
                ? '商品は取り置き済みです。期限内にお振込みください。'
                : 'Your items are reserved. Please pay before the deadline.'}
            </p>
