...
async function setupLiveStream(page: import('@playwright/test').Page) {
  ...
  const activateBtn = page.getByRole('button', { name: '\u8868\u793a\u4e2d' }).first()
  if (await activateBtn.isVisible().catch(() => false)) {
    await activateBtn.click()
    await page.waitForTimeout(500)
  }
  const m = page.url().match(/\/admin\/live\/(\d+)/)
  expect(m).toBeTruthy()
  return { streamId: m![1], title }
}
...
test('01 admin enables offers, public submit, admin realtime', async ({ page }) => {
  ...
  await publicPage.getByTestId('offer-amount-input').fill('1600')
  ...
  await expect(page.getByTestId('admin-offers-list')).toContainText('\u00a51,600', { timeout: 30_000 })
  await shot(page, '01-admin-realtime-offer')
...
test('02 submit hold accept user status', async ({ page }) => {
  ...
  await ensureOffersOn(page)
  const publicPage = await publicLivePage(page, streamId)
  await publicPage.getByTestId('offer-amount-input').fill('1700')
  await publicPage.getByTestId('offer-submit').click()
  await expect(page.getByTestId('admin-offers-list')).toContainText('\u00a51,700', { timeout: 30_000 })
...
test('03 submit reject user sees rejected', async ({ page }) => {
  ...
  await ensureOffersOn(page)
  const publicPage = await publicLivePage(page, streamId)
  await publicPage.getByTestId('offer-amount-input').fill('1800')
  await publicPage.getByTestId('offer-submit').click()
  await expect(page.getByTestId('admin-offers-list')).toContainText('\u00a51,800', { timeout: 30_000 })
...
test('04 accept purchase at accepted price', async ({ page }) => {
  ...
  await publicPage.getByTestId('offer-submit').click()
  await expect(page.getByTestId('admin-offers-list')).toContainText('\u00a51,900', { timeout: 30_000 })
  ...
  await expect(publicPage.getByTestId('offer-my-status')).toContainText('\u6ce8\u6587', { timeout: 30_000 })
  await shot(publicPage, '04-purchase-complete')
  await publicPage.close()
})
