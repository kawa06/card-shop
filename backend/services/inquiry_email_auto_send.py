"""Inquiry email auto-send preferences (admin-configurable per event)."""
...
from services.inquiry_email_registry import all_auto_send_defaults
...
def get_auto_send_settings(db: Session) -> dict[str, bool]:
    ...
    raw = getattr(brand, "inquiry_email_auto_send_json", None) or ""
    if not raw:
        return defaults
    try:
        ...
        logger.warning("Invalid inquiry_email_auto_send_json")
    return defaults
...
def update_auto_send_settings(db: Session, updates: dict[str, bool]) -> dict[str, bool]:
    ...
    for key, value in updates.items():
        if key in current and isinstance(value, bool):
            current[key] = value
    brand = get_brand_settings(db)
    brand.inquiry_email_auto_send_json = json.dumps(current, ensure_ascii=False)
    return current
