...
def _favorite_query(db: Session):
    return db.query(models.Favorite).options(joinedload(models.Favorite.card))
...
@router.post("/{card_id}", response_model=schemas.FavoriteActionOut, status_code=status.HTTP_201_CREATED)
def add_favorite(
    card_id: int,
    current_user: models.User = Depends(get_current_user),
    db: Session = Depends(get_db),
